...
export default class ModuleInstance extends InstanceBase<ModuleSchema> {
	...
	getReplicants(): ReplicantRecord[] {
		return [...this.replicants.values()].sort((a, b) =>
			replicantKey(a.bundle, a.name).localeCompare(replicantKey(b.bundle, b.name)),
		)
	}
	...
	private applySnapshot(items: ReplicantSnapshot[]): void {
		const oldVariableIds = new Set(
			[...this.replicants.values()].map((item) => variableIdForReplicant(item.bundle, item.name)),
		)
